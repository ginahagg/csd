...
save_snippet(Snippet) ->
  pooler:take_member(fun(RiakPid) -> csd_snippet_store:save(RiakPid, Snippet) end).

get_snippet(SnippetKey) ->
  pooler:take_member(fun(RiakPid) -> csd_snippet_store:fetch(RiakPid, SnippetKey) end).

list_snippets(UserId, FieldsToKeep, PageSize, PageNumber) ->
  pooler:take_member(fun(RiakPid) ->
...
get_comment(CommentId) ->
  pooler:take_member(fun(RiakPid) -> csd_comment_store:fetch(RiakPid, CommentId) end).

save_comment(Comment) ->
  pooler:take_member(fun(RiakPid) -> csd_comment_store:save(RiakPid, Comment) end).

list_all_comments(SnippetId) ->
  pooler:take_member(fun(RiakPid) -> csd_comment_store:list_all(RiakPid, SnippetId) end).

list_comments_since(SnippetId, Since) ->
  pooler:take_member(fun(RiakPid) -> csd_comment_store:list_all_since(RiakPid, SnippetId, Since) end).
...
get_user(UserId) ->
  pooler:take_member(fun(RiakPid) -> csd_user_store:fetch(RiakPid, UserId) end).

save_user(User) ->
  pooler:take_member(fun(RiakPid) -> csd_user_store:save(RiakPid, User) end).
...
get_vote(VoteId) ->
  pooler:take_member(fun(RiakPid) -> csd_vote_store:fetch(RiakPid, VoteId) end).

save_vote(Vote) ->
  pooler:take_member(fun(RiakPid) -> csd_vote_store:save(RiakPid, Vote) end).
